...
class Step2Panel(BoxLayout):
    ...
    def send_global_data(self):
        ...
        try:
            ...
            MessageSender.send(
                "ERLANG_REQUEST", payload, callback=self._on_erlang_response
            )
    ...
    def _on_erlang_response(self, response):
        ...
        try:
            print(
                f"DEBUG: Respuesta recibida tipo: {type(response)}, contenido: {response}"
            )
    ...
    def show_erlang_results(self):
        ...
        print(
            f"DEBUG show_erlang_results: hasattr erlang_results_data = {hasattr(app, 'erlang_results_data')}"
        )
        if hasattr(app, "erlang_results_data"):
            print(
                f"DEBUG show_erlang_results: erlang_results_data = {app.erlang_results_data}"
            )

        form = GridForm(cols=2)

        erlangs = (
            app.erlang_results_data.get("Erlangs", "---")
            if hasattr(app, "erlang_results_data")
            else "---"
        )
        max_lines = (
            app.erlang_results_data.get("maxLines", "---")
            if hasattr(app, "erlang_results_data")
            else "---"
        )
        ...
        popup = ConfigPopup(
            title_text="Softphone (Destino) - Resultados Erlang", content_widget=form
        )
        popup.open()
